...
In Section \ref{sec:requirements-initial} a number of high-level goals for ideal markup language transformation programs were outlined. This section will build upon the theory outlined in both Section \ref{sec:theory} as well as \ref{sec:theory-of-document-processing}. Using this body of knowledge as stepping stone, this chapter will formulate a set of requirements for an ideal markup transformation program.

The high-level goals outlined in Section \ref{sec:requirements-initial} were that the transformation program must (1) be able to produce ... TODO.

\section{Separating intents}


\subsection{Semantics}
TODO: Derive ideas from the GML to SGML transformation where it is realized that DTD's and DocTypes are not the same thing. Tag-validity, and type-validity.

\subsection{Validations}
TODO: Build upon ideas from the latter, and the concepts of the DTD.

\subsection{Derivations}
TODO: Build upon the ideas of Scribe, chap. 7.1 Derived Text (p. 78).

Also Krijnen exhibit these ideas in the suggestion to extend Pandoc. They also consider the case of packages. And the idea that packages should be shared before deciding on an output format.

\subsection{Presentations}
TODO: Draw ideas from Pandoc, and also build upon the Krijnen discussion from the last section.
...
\section{Refining Quadrant B programs}
As mentioned earlier in this chapter, the quadrant view is an oversimplification and of course there are many interesting nuances within one quadrant. This section will refine Quadrant B programs using the initially proposed requirements for a transformation program (outlined in Section \ref{sec:requirements-initial}).

...
Even today, hierarchical document structures enjoy the most widespread use. Consider, for example, the large family of XML-based languages such as HTML.
...
More importantly, the fact that two words are underlined, whereas one of the words that are underlined is actually both underlined and bold. This is an example of a sitaution where nesting help us express this significantly easier.  Consider for example how the string is represented as a tree (i.e. a hierarchical structure) in Figure \ref{fig:mixed-content-tree}). To understand why nesting is a sensible approach to such situations consider the lexing step of a compiler that read the paragraph in question. Without nesting, the number of terminal tokens needed in the lexer would have to be the number of combinations of terminal tokens before combining.
...
\paragraph{(1) There is no semantic difference between an element implied by the use of nesting rather than it's explicit counterpart.}
...
\paragraph{(2) No significantly complex cognitive leap is required for humans in order to be able to think and discuss nesting.}
Foobar.

\paragraph{(3) Nesting can reduce the number of required terminal tokens in a markup compiler exponentially.}
Foobar.

\paragraph{(4) nesting allow languages to contain an infinite set of semantic tokens while expressing only a finite set of tokens in the compiler.}
Foobar.

In other words, assume two unique terminals -- A and B. If one is to allow a string to at any time be described as belonging to the class of A, or the class B, or to the class of both A and B simultaneously. Without nesting this requires a third token -- C. However, since humans seems to require no absurd cognitive gymnastics in order to think of a piece of string as belonging to two classes rather than a new joint class one might as well choose to encode this as a nested for humanto be cognitively natural for humans to think needed in our lexer would have to be the number of permutations of \texttt{n} (i.e. \(n!\)) where \(n\) is the number of unique, uncombined tokens.
...
\centering
\fbox{
Lexing \underline{without \textbf{\sout{parsing} often}} make \textbf{life \sout{oh so} \underline{cumbersome}}.
}

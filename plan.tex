...
\section{Proposing a DSL for generating markup-oriented PEG}
...
Asking semi-technicals (whom authors very well may be) to define languages using Formal Grammars such as a Parsing Expression Grammar (PEG) is however of course not only absurd, but also time-consuming and error prone. Thus the suggestion is to create a small DSL and a conversion program that converts this simpler language to actual PEG.

This smaller language will of course limit the number of expressible languages in PEG such that all valid inputs in the language will when run through the program produce an PEG that conform the set of requirements outlined in this chapter, and is thus suitable as a markup format.



\subsection{A DSL for generating markup-oriented PEG}
...
\subsubsection{Example of a generated PEG}

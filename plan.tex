...
\section{Transformation program requirements}
...
\paragraph{The model of Quadrant B} (\ref{fig:workflows-framework-spec-in-gen-out}) will be used as a starting point due to the arguments given in \ref{chpt:derived-transformation-requirements}. In summary, it is the one of the four quadrant models that fit the initial high-level goals outlined in Section \ref{sec:requirements-initial}.


\section{Applying the three-step model Quadrant B}
As the three-step model of \citet{goldfarb}, discussed in Section \ref{sec:three-step-model}, is (as mentioned) quite versatile, it will in this section be applied to the Quadrant B model of the micro-framework discussed in Section \ref{sec:microframework}.
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[node distance=.15cm]
    ...
    \node (C2)  [io,      above of=T2, yshift=2cm, xshift=-1cm] {\(C2_n\)}; % {Validations};
    \node (F2)  [io,      above of=T2, yshift=2cm, xshift=1cm] {\(F2_n\)}; % {Validations};

    \node (C3)  [io,      above of=P3, yshift=2cm] {\(C3_n\)}; % {Derivations};
    \node (C4)  [io,      above of=P4, yshift=2cm] {\(C3_n\)}; % {Presentation};
  ...
  \caption{Suggested ideal refinement of a Quadrant B program.}
  \label{fig:workflows-framework-ideal}
\end{figure}

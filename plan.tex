...
\section{Proposing a DSL for generating markup-oriented EBNF}
In order to fulfill the requirement of Generic Syntax, the author of this thesis propose that document authors use Formal Grammars to express a language specific for her particular domain. A language in which the end content then can be marked up.

Asking semi-technicals (whom authors very well may be) to define languages using Formal Grammars such as EBNF is however of course not only absurd, but also time-consuming and error prone. Thus the suggestion is to create a small DSL and a conversion program that converts this simpler language to actual EBNF.

This smaller language will of course limit the number of expressible languages in EBNF such that all valid inputs in the language will when run through the program produce an EBNF that conform the set of requirements outlined in this chapter, and is thus suitable as a markup format.



\subsection{A proper, markup-oriented subset of Regular Expressions}


\subsection{A DSL for generating markup-oriented EBNF}


\subsubsection{Example of a generated EBNF}

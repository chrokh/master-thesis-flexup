...
More importantly, the fact that two words are underlined, whereas one of the words that are underlined is actually both underlined and bold. This is an example of a sitaution where nesting help us express this significantly easier.  Consider for example how the string is represented as a tree (i.e. a hierarchical structure) in Figure \ref{fig:mixed-content-tree}). Without nesting, the number of tokens needed in our lexer would have to be the number of permutations of \texttt{n} (i.e. \(n!\)) where \(n\) is the number of unique, uncombined tokens.

...
\paragraph{The micro-framework stem from the following argument: } To perform the process, previously described as transforming from $F$ to $F\prime$, one can trivially distinguish the three parts of the statement. The pre-state ($F$), the post-state ($F\prime$), and the $program$. In Figure \ref{fig:workflows-framework} this is depicted in that \(F\) is referred to as ``Input file'' and \(F\prime\) as ``Output file'', and the program as the ``Transformation program''. Again, as this model simply depicts the transformation as a single joint function there is really no need for arguing its validity. The model simply depicts a function where the domain of all possible inputs is given as input, and the domain of all possible outputs is produced as output. Since the very nature of a markup transformation program is to take a string of some language and produce a string in perhaps another language, then of course all markup transformation programs fall under this model.

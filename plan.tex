...
Now, consider the fact that the four types of terminal patterns in the example above, only corresponds to the four semantically different patterns expressed in the example. Assuming the three base terminals plain-text, underline, bold, and strike-through. In fact, if one were to write a proper lexer covering all of the legal strings in the language, the number of terminal patterns would actually have to be \(n!\) -- i.e. the number of permutations of the number of base tokens. This is expressed more clearly in Figure \ref{fig:mixed-content-paragraph-complex}.

...
In XML, the author of a document is given the freedom to markup her document with whatever element names the author see fit\footnote{http://www.w3.org/TR/WD-xml-961114}. As XML is a subset of SGML\footnote{http://www.w3.org/TR/WD-xml-961114}, and SGML ascended from XML\footnote{http://www.sgmlsource.com/history/roots.htm}, it is only reasonable to assume that this concept was already present in the earlier language GML. In GML, this concept was actually referred to as Generic Identifiers (GI's) \citet{goldfarb}.
...
The ingenious move of GML was to let the markup language only go as far as point one above \citet{goldfarb}. In other words, GML allow an author describe a document's structure in any way she see fit, using the mnemonic tag -- i.e. a GI.

In order to achieve the second goal, \citet{goldfarb} explained that GML documents ought to be coupled with GML Models. Where the GML Model notation essentially is a notation derived from the BNF-notation, but where the intent is used to define the set of all valid elements. Perhaps this is the forefather of the Document Type Definition (DTD) that were introduced into SGML and followed into the subset XML\footnote{http://www.w3.org/TR/WD-xml-961114\#dt-dtd}.
...
\citeauthor*{goldfarb}, one of the authors of IBM Generalized Markup Language (GML), proposed (in \citeyear{goldfarb}) a three-step model for understanding document processing. The term document processing here refer to the idea of taking some document containing Generic Identifier's (GI's) as input, applying some set of processing instructions, to produce some other document as output.

The term Generic Identifier \citet{goldfarb} refer

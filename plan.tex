...
In order to change the model such that it can depict, what previously in the thesis was referred to as, n-to-n conversions, we will introduce the notion of coupling. In Figure \ref{fig:workflows-framework-area} we've introduced a dotted line. This dotted line represents coupling. If any of the components within the area delimited by the dotted line change, the change will ripple through all of the other components. Assume for example that the input file and the transformation program are both within the dotted area. This would mean that to handle a new input we need a new transformation program. Which is of course unwanted.

\subsection{Modeling configurability}
In order for the model to also encompass the idea of configurability we will extract one piece of the transformation program and thus describe it in further detail. What we will extract is a configuration file (see Figure \ref{fig:workflows-framework-area}). Essentially the model of the program can now be seen as a function that takes two arguments and produces one result. An input file, a configuration file that give the end-user some level of control over the transformation process.
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[node distance=2.5cm]
    ...
    \begin{pgfonlayer}{background}
      \node [draw, dotted, rectangle, rounded corners, fit={(input) (engine) (output)}] { };
    \end{pgfonlayer}

  \end{tikzpicture}

  \caption{The dotted line depict coupling, such that if any of the components within the area change, changes will have to be carried out in all the other components within the area.}
  \label{fig:workflows-framework-area}
\end{figure}

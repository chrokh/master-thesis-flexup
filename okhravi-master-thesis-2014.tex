...
\paragraph{Given the following input document}:

\begin{lstlisting}
[q]This **string** exhibits _both **nesting** and **mixed content**_[/q]
\end{lstlisting}


\paragraph{And this language description}:
...
\paragraph{We reach this intermediate flXML}:
...
\section{Numbered headings example}
...
\paragraph{So if there exist an XSLT package for numbered headings}:
\begin{lstlisting}
<?xml version="1.0" encoding="utf-8"?>
<xsl:stylesheet version="1.0"
xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
<xsl:template match="//numberedHeading">
  <numberedHeading>
    (<xsl:value-of select="count(preceding-sibling::numberedHeading)+1"/>)
    <xsl:value-of select="." />
  </numberedHeading>
</xsl:template>
</xsl:stylesheet>
...
\paragraph{We can translate our document so that the domain names of our document match the domain names of the package}, using the following configuration:
\begin{lstlisting}
{
...
\paragraph{And thus we reach the following output document}:
\begin{lstlisting}
<heading>(1) First heading </heading>
And then some text

<heading>(2) Second heading </heading>
And some more text

<heading>(3) Third heading </heading>
And the last paragraph
\end{lstlisting}
...
\paragraph{And all of these steps could be carried out using the following orchestration file}:
...
\paragraph{And if we kept on going we could go on an output this document as html}:
...
Which would bring us the output (numbered.html):
\begin{lstlisting}
<heading>(1) First heading </heading>
And then some text

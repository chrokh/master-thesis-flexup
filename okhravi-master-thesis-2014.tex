...
\paragraph{As an example} consider the four previously mentioned categories. One could argue, that the idea of presentation is a specialization, where the idea of describing a thing outside its domain of presentation is an abstraction. This would mean, that formats concerned with presentation are less abstract (i.e. more specialized) than those not concerned with presentation.

Be the hypothesis true, this would mean that converting from a non-presentational format to a presentational format, should be considered a conversion ``down'' the abstraction chain. Thus a significantly easier task, that it's counterpart -- converting from a presentational format to a non-presentational format. As the latter would be a conversion ``up'' the abstraction chain.

The factuality of this statement becomes obvious when considering the well-accepted notion of ``separating presentation from content''. Or more broadly expressed, the notion of ``separation of concerns''. However, the hypothesis of this thesis is of course not only that presentation and content are different. But again, that all languages exhibit properties, and that all of these properties can grouped into categories, where these categories can be ordered hierarchically according to their level of abstraction.

\paragraph{The chain of abstractions} is perhaps better explained by analogy of a tree, rather than a chain. Consider Figure \ref{fig:fictive-specialization-tree}, in which the language $L$ is the most abstract. While the languages ``below'' are specializations of that language. It is important to understand that, like in any hierarchy, branchings may exist. Which becomes apparent through considering the siblings $L1$, $L2$ and their relationship. However, it is also important to remember that the hypothesis suggest to only allow conversions from parent to child (i.e. ``down'') and never from child to parent, nor from child to child.

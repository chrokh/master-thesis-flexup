...
\paragraph{To summarize, this thesis will concern itself with three types of conversions -- conversions from input language (i.e. encoding) to interpretation, conversions from interpretations to interpretations, and conversions from interpretation to output language.} In other words, this thesis will concern itself only very briefly with conversions from the abstract idea to encodings (i.e. input languages). 




\color{red}
\section{languages are subsets of languages}
TODO
\color{black}
...
\paragraph{\color{red}Retainment of distinction must be determined on the basis of languages, not languages, nor instances of languages.}
...
\paragraph{Attempting ``M-to-N'' instead of ``N-to-N''} seem to be another way to look at this ``walking in the right direction'' of the abstraction chain. If one starts ($M$) at the intersection of all possible languages (all $N$) one may want to output to, then it is only reasonable to assume that the level of abstraction will be utterly high from the beginning. Subsequently then decrease as transformations move it closer and closer to concrete languages.

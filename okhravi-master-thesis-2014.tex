...
\chapter{Background}
The problem approached in this thesis, is perhaps best summarized by the quote raised in a paper by \citet{krijnen}. That paper, however, approach the problem from a different angle than this thesis. That paper employ the $n-to-n$ approach, whereas this thesis explore an $m-to-n$ approach.
...
For many good reasons, different formats exist. Simple things such as taste, cause different languages to express the same things through different syntactical constructs. Languages offer different levels of control. At different levels of abstraction.

One can informally categorize document formats into three groups on the basis of design intentions. A language may have been designed with the intention of enabling document authors to somewhat trivially, express documents in the language manually. On the other end of the spectra a format may have been designed without this intent, perhaps because it is aiming for very high precision. The second factor to consider is whether a format was designed to directly produce some visual output. With these two aspects the following four categorizations emerge.
...
Disregarding all categorizations of document annotation formats, this transformation becomes an $n-to-n$ problem. An existing approach to this ``n-to-n'' conversion problem, and perhaps the most obvious one, is Pandoc\footnotePandoc -- dubbed ``a universal document converter''.

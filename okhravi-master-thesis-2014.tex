...
%TODO? \section{Markup has resolution}
...
\paragraph{There exist not always a single, true, output document, per language, for every given input document.} The reason multiple different documents within a single language could be considered valid output for a given input document is because of dialects within a language. Much as in natural language, the same thing, can be said in different ways. Sometimes semantics differ between two different ways, sometimes they do not.

It is pragmatic to allow a single input document, to be converted into multiple different output documents of the same language. One could of course argue, that if there exist, at least one, semantic difference between two output documents, then these two documents cannot possibly have been derived from the same input document. Subsequently, one would reach the conclusion that these output documents, in fact, are either (a) expressed in two different languages, or (b) derived from two different input documents. As this question tumbles down the depths of linguistics, this thesis will simply choose the pragmatic approach of alternative (a). In other words, the approach that admit the existence of ambiguous interpretation of input documents.

In order to exemplify the pragmatic usefulness of accepting ambiguity, consider an example of a document consisting of a set of paragraphs, intended to be converted to HTML. It may be equally reasonable to encode this list of paragraphs, as either a list of \texttt{<li>} elements (i.e. list items), or simply a set of subsequent \texttt{<p>} elements (i.e. paragraph elements). Since there exist a semantic overlap, as depicted in Figure TODO, between the two elements, it is entirely possible that the author of the original document referred to some semantics that happens to lie in the intersection between the two interpretations of the output format. In other words, this problem can be argued to stem from ambiguity of output languages such as HTML.

Whether such semantic ambiguity, or rather semantic overlap, is even practically possible to eliminate is left unsaid. This thesis merely admit that it is not possible to claim that there only ever exist one correct interpretation in a given language for a given input document.


\paragraph{Another, more pragmatic, reason as to why one may want to convert a single input document into multiple different documents within the same language is that of efficiency.} As the main purpose of this thesis is to support the notion of ``write once, publish everywhere'', then it is only reasonable to assume that a common scenario will be to convert a given document, expressed in some language, to variations of that document.

Consider for example a book, expressed in some language, intended to be published in HTML. The author may for example wish to publish two versions of the book. In one version all of the chapters sequentially follow each other on the same page, and in the other version all chapters are divided into separate pages, and a table of contents that contain hyperlinks to all the pages is also generated. While both of the output documents are expressed in language of HTML, they are slight variations of the original document.


\paragraph{TODO}
In other words, assume there exist an abstract idea in the mind of a document author. Assume then that the author encodes this idea using some input language. The author of this thesis holds the philosophical standpoint that the concrete input document (i.e. the encoded idea) is always a compromise (i.e. approximation) of the actual idea. However the intent of the input format is to approximate the abstract idea as closely as possible. The intent of the conversion tool is then to translate this approximation of the abstract idea in the input language, into an approximation of the approximation in another language. Clearly, also the goal of the conversion tool must be to achieve an approximation as close to the original (in this case, another approximation) as possible.

 reason as to why multiple output documents in a given language may exist for a given input document, is simply the need for multiple output documents of the same language. 
 


\citet{krijnen} suggest an even less naive approach, by demonstrating how conversion schemes can be shared across different input and output formats, if they operate on the intermediate format. An interpretation of that suggested approach is depicted in Figure \ref{fig:interpretation-of-krijnen-extended-pandoc}. While such an approach does still require at least $2n$ number of conversion schemes, the benefit lies in configuration. TODO!
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[node distance=1cm]
      ...
      \node (ext) [dot, above of=mid, yshift=1cm] {$E_i$};
    ...
    \caption{Interpretation of the extended Pandoc system by \citet{krijnen}, enabling package sharing in $n-to-n$ conversion.}
    \label{fig:interpretation-of-krijnen-extended-pandoc}
  \end{figure}

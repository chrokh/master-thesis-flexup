...
\paragraph{Recognition} refer to the idea of recognizing that some piece of the document string should be considered an element that is marked up. While not explicitly expressed by \citet{goldfarb} it seems reasonable to informally consider this step to be the step of lexing (tokenization) and parsing, where its output would to be the parse tree. \citet{goldfarb} gives the example of an element with the generic identifier ``footnote''. In this step, the processing program would know that a substring is to be considered a ``footnote'', and, is thus semantically differentiated from all things that are not ``footnotes''. While the semantic delimitation is recognized in this step, nothing is known about what this particular semantic delimitation means in terms of processing. 

\paragraph{Mapping} refers to the idea of pairing processing functions with document elements (or to follow the analogy of the last paragraph -- nodes in the parse tree -- or semantic delimitations). \citet{goldfarb} continue on the previously mentioned example of ``footnotes'', by saying that the element now would be paired up with a processing function, such as e.g. one that collects the content encapsulated in the footnote element, only to later print it at the bottom of that page.

\paragraph{Processing} simply refer to the task of actually executing processing functions upon their respectively mapped elements.

\paragraph{In terms of the XML eco-system,} recognition is essentially done in all XML parsers, whereas mapping would be conducted in a language such as XSLT, where XPath could be used for traversing the parsed XML tree in order to find a particular element one wish to map to a processing function expressed in XSLT. Finally, the step of processing would be carried out by an XSLT processor such as, e.g. SAXON.

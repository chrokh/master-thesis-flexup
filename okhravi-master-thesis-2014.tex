...
Two common, and less naive, approaches to $n-to-n$ conversion are that of Pandoc, and that of \citet{krijnen}. The former constructs conversion schemes on a per-language basis, while the latter complements this with the realization that some schemes may actually be constructed on a per-category-basis.

Pandoc, make use of an intermediate format, which essentially mean that merely two conversion schemes has to be written per language. One that converts from the language to the intermediate format, and one that converts from the intermediate format to the language. This essentially decrease the (otherwise exponentially growing) need for unique conversion schemes to merely $2n$. Thus 10 formats would require 20 conversion schemes. Generally visualized in Figure \ref{fig:n-to-n-conversion-boxes-with-intermediate-format}.
